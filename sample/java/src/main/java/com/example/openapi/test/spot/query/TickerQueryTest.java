...
public class TickerQueryTest {
     ...
     */
    public TickerVO getTickerData(String symbol) throws SavicoinApiException {
        try {
            // 验证必填参数
            ...
            if (apiResponse.getCode() != 0) {
                throw new SavicoinApiException("获取Ticker数据失败: " + apiResponse.getMsg());
            }
            ...
            throw new SavicoinApiException("获取Ticker数据时出错: " + e.getMessage(), e);
        }
    }
     ...
     * 测试获取ticker数据
     */
    private void testGetTickerData() throws SavicoinApiException {
