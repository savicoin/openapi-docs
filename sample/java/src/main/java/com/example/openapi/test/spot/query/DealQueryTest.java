...
public class DealQueryTest {
     ...
     */
    public List<DealVO> getDealData(String symbol, Integer num) throws SavicoinApiException {
        try {
            // 验证必填参数
            ...
            if (num == null || num < 1) {
                throw new SavicoinApiException("数量必须大于等于1");
            }
            ...
            if (apiResponse.getCode() != 0) {
                throw new SavicoinApiException("获取成交数据失败: " + apiResponse.getMsg());
            }
            ...
            throw new SavicoinApiException("获取成交数据时出错: " + e.getMessage(), e);
        }
    }
     ...
     * 测试获取最新成交信息
     */
    private void testGetDealData() throws SavicoinApiException {
        log.info("===== 获取最新成交信息测试 =====");
