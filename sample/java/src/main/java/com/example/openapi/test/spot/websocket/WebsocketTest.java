...
public class WebsocketTest {
    ...
    public static void main(String[] args) {
        try {
            // 构建客户端实例
            SavicoinWebSocketClient client = createWebSocketClient();
     ...
     */
    private static SavicoinWebSocketClient createWebSocketClient() throws URISyntaxException {
        // 构建 WebSocket URL
        String wsUrl = String.format("%s/spot/v1/ws/socket", WS_HOST);
        Console.log("正在连接 WebSocket: {}", wsUrl);

        return new SavicoinWebSocketClient(new URI(wsUrl));
    }
    ...
    private static String getWebSocketToken() {
        try {
            ...
            // 构建签名 - 使用正确的方法
            String signature = SavicoinApiUtils.generateSignature(
                    SECRET_KEY,
                    sortedParams,
     ...
     */
    private static class SavicoinWebSocketClient extends WebSocketClient {
        private Timer heartbeatTimer;
        private String symbol = "BTC_USDT";

        public SavicoinWebSocketClient(URI serverUri) {
            super(serverUri);
        }
