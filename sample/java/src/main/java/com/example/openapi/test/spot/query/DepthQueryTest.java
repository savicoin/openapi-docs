...
public class DepthQueryTest {
     ...
     */
    public DepthVO getDepthData(String symbol, Integer level) throws SavicoinApiException {
        try {
            // 验证必填参数
            if (symbol == null || symbol.isEmpty()) {
                throw new SavicoinApiException("交易对不能为空");
            }

            if (level == null) {
                throw new SavicoinApiException("档位不能为空");
            }

            // 验证level范围
            if (level < 1 || level > 50) {
                throw new SavicoinApiException("档位必须在1到50之间");
            }
            ...
            if (apiResponse.getCode() != 0) {
                throw new SavicoinApiException("获取深度数据失败: " + apiResponse.getMsg());
            }
            ...
            throw new SavicoinApiException("获取深度数据时出错: " + e.getMessage(), e);
        }
    }
     ...
     * 测试获取深度数据
     */
    private void testGetDepthData() throws SavicoinApiException {
        log.info("===== 获取深度数据测试 =====");
