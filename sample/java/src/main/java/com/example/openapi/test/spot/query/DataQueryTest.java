...
public class DataQueryTest {
    ...
    public List<KlineVO> getKlineData(String symbol, String interval, Integer limit,
                                      Long startTime, Long endTime) throws SavicoinApiException {
        try {
            // 验证必填参数
            ...
            if (interval == null || interval.isEmpty()) {
                throw new SavicoinApiException("时间间隔不能为空");
            }
            ...
            if (limit != null) {
                ...
                if (limit < 1 || limit > 1500) {
                    throw new SavicoinApiException("limit必须在1到1500之间");
                }
            ...
            if (apiResponse.getCode() != 0) {
                throw new SavicoinApiException("获取K线数据失败: " + apiResponse.getMsg());
            }
            ...
            throw new SavicoinApiException("获取K线数据时出错: " + e.getMessage(), e);
        }
    }
     ...
     * 测试获取最近的K线数据
     */
    private void testGetKlineData() throws SavicoinApiException {
        log.info("===== 获取K线数据测试 =====");
