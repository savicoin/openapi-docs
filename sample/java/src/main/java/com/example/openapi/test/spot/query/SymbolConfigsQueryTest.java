...
public class SymbolConfigsQueryTest {
     ...
     */
    public List<SymbolConfigVO> getSymbolConfigs() throws SavicoinApiException {
        try {
            // 创建查询参数Map (此接口无需参数)
            ...
            if (apiResponse.getCode() != 0) {
                throw new SavicoinApiException("获取币种配置失败: " + apiResponse.getMsg());
            }
            ...
            throw new SavicoinApiException("获取币种配置时出错: " + e.getMessage(), e);
        }
    }
     ...
     * @throws SavicoinApiException 如果API调用失败
     */
    public SymbolConfigVO getSymbolConfigBySymbol(String symbol) throws SavicoinApiException {
        if (symbol == null || symbol.isEmpty()) {
            throw new SavicoinApiException("交易对符号不能为空");
        }
     ...
     * 测试获取币种配置信息
     */
    private void testGetSymbolConfigs() throws SavicoinApiException {
        log.info("===== 获取币种配置信息测试 =====");
     ...
     * 测试查询特定交易对配置
     */
    private void testGetSpecificSymbolConfig() throws SavicoinApiException {
