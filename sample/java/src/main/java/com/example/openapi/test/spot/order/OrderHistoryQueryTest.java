...
public class OrderHistoryQueryTest {
     ...
     * 测试查询BTC_USDT最近订单历���
     */
    private void testQueryRecentOrders() throws SavicoinApiException {
        log.info("===== 测试查询最近订单历史 =====");
     ...
     * 测试按条件查询订单历史
     */
    private void testQueryOrdersWithConditions() throws SavicoinApiException {
     ...
     * 分页查询测试
     */
    private void testPaginationQuery() throws SavicoinApiException {
        log.info("===== 测试分页查询订单历史 =====");
     ...
     * 测试查询已取消的订单
     */
    private void testQueryCanceledOrders() throws SavicoinApiException {
        log.info("===== 测试查询已取消订单 =====");
     ...
     * 测试查询杠杆账户订单
     */
    private void testQueryLeverageOrders() throws SavicoinApiException {
        log.info("===== 测试查询杠杆账户订单 =====");
    ...
    public ScrollPageResult<OrderVO> getHistoryOrders(
        ...
        try {
            ...
            if (jsonResponse.getInt("code") != 0) {
                throw new SavicoinApiException("查询历史订单失败: " + jsonResponse.getStr("msg"));
            }
            ...
            throw new SavicoinApiException("查询历史订单时发生错误: " + e.getMessage(), e);
        }
    }
