...
public class UnfinishedOrderQueryTest {
    ...
    public PageResult<OrderVO> getUnfinishedOrders(
        ...
        try {
            ...
            if (apiResponse.getCode() != 0) {
                throw new SavicoinApiException("查询未完成订单失败: " + apiResponse.getMsg());
            }
            ...
            throw new SavicoinApiException("查询未完成订单时出错: " + e.getMessage(), e);
        }
    }
     ...
     * 测试查询未完成订单
     */
    private void testQueryUnfinishedOrders() throws SavicoinApiException {
        log.info("===== 测试查询未完成订单 =====");
     ...
     * 测试使用特定条件查询未完成订单
     */
    private void testQueryWithConditions() throws SavicoinApiException {
