...
public class OrderCancelTest {
     ...
     */
    public String cancelOrder(Long orderId) throws SavicoinApiException {
        try {
            // 验证参数
            ...
            if (!apiResponse.isSuccess()) {
                throw new SavicoinApiException("撤销订单失败: " + apiResponse.getMsg());
            }
            ...
            throw new SavicoinApiException("撤销订单时出错: " + e.getMessage(), e);
        }
    }
     ...
     * @throws SavicoinApiException 如果API调用失败
     */
    private String createLimitOrder(String symbol, String direction, BigDecimal amount, BigDecimal price) throws SavicoinApiException {
        try {
            // 创建订单请求对象
            ...
            if (!apiResponse.isSuccess()) {
                throw new SavicoinApiException("创建订单失败: " + apiResponse.getMsg());
            }
            ...
            throw new SavicoinApiException("创建订单时出错: " + e.getMessage(), e);
        }
    }
     ...
     * 测试撤销买单
     */
    private void testCancelBuyOrder() throws SavicoinApiException {
        log.info("===== 测试撤销买单 =====");
     ...
     * 测试撤销卖单
     */
    private void testCancelSellOrder() throws SavicoinApiException {
        log.info("===== 测试撤销卖单 =====");
